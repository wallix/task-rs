...
overridable, use the `default` filter:
<span v-pre>`MY_VAR: '{{ MY_VAR | default("my-default-value", true) }}'`</span>.
The `true` widens it to substitute for an empty override as well as an unset
one; without it only an unset one takes the fallback.
...
tasks:
  greet:
    vars:
      RECIPIENT: '{{ RECIPIENT | default("World") }}'
    cmds:
...
You can use template expressions in `if` conditions. Expressions like
<span v-pre>`{{ VAR == "value" }}`</span> evaluate to `true` or `false`, which are valid shell
...
failing, enable interactive mode with the `--interactive` flag:
...
You can enable prompts from the command line with `--interactive`.
...
If `cache.lock` is configured with a `redis://`, `vk://` or `vks://` URL, Task
uses a distributed lock with automatic heartbeat renewal. If that server is
...
If a distributed lock is lost while the task runs — the server disowns the
holder, or a whole lease TTL passes with no renewal confirmed — Task warns and
publishes neither the fingerprint nor the cache entry: a peer may have been
building the same task at the same time, so the next run re-verifies the task
instead of trusting a result that may not have been built exclusively.
...
Values in a Taskfile are rendered with a template engine. Task defaults to the
**Jinja** dialect (`{{ VAR }}`, `{% if %}`, filters), and the examples in this
section use it. The legacy Go `text/template` dialect (`{{ VAR }}`) is still
supported but **deprecated** — run `task --migrate` to convert a Taskfile. See
the [templating reference][templating-reference] for full details.
...
tasks:
  ...
  bar:
    cmds:
      - 'echo {{ FOO[0] }}' # <-- FOO is a string, so this reads its first character '[' instead of the expected 'A'
```
...
tasks:
  ...
  bar:
    cmds:
      - 'echo {{ FOO[0] }}' # <-- FOO is still an array so the task outputs 'A' as expected
```
...
tasks:
  ...
  bar:
    cmds:
      - 'echo {{ BAR[0] }}' # <-- BAR still refers to FOO so the task outputs 'A'
```

All references use the same templating syntax as regular templates, so in
addition to naming `FOO`, you can also pass subkeys (`FOO.BAR`), indexes
(`FOO[0]`) and the result of expressions, as described in the
  ...
  foo:
    ...
    cmds:
      - task: bar
        vars:
          FOO:
            ref: FOO[0] # <-- The element at index 0 is passed by reference to bar
  bar:
    cmds:
...
tasks:
  default:
    cmds:
      - defer:
          echo '{% if EXIT_CODE %}Failed with {{ EXIT_CODE }}!{% else %}Success!{% endif %}'
      - exit 1
```
...
[shell interpreter](https://github.com/reubeno/brush) that Task uses.
...
[report any issues](https://github.com/wallix/task-rs/issues/new)
to us.
