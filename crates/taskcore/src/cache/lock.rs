...
impl Guard {
    /// Whether the lock is known to have been lost while held — the remote
    /// backend disowned us, or its lease/TTL lapsed with no renewal getting
    /// through. From that moment a peer may hold the same name, so anything
    /// this guard protects is no longer exclusive: a caller about to write
    /// shared state (the fingerprint, the remote cache) should check this and
    /// skip the write.
    ///
    /// Always `false` for a file lock: there is no lease to lapse, and a peer
    /// only reclaims the lockfile once the PID recorded in it is gone (see
    /// `evict_stale_lock`). `false` from a remote backend means "no evidence of
    /// loss", not proof of holding.
    pub fn is_lost(&self) -> bool {
        match self {
            Guard::File(_) => false,
            Guard::Vk(lease) => lease.as_ref().is_some_and(ocicas::Lease::is_lost),
            Guard::Redis(guard) => guard.as_ref().is_some_and(RedisGuard::is_lost),
        }
    }
...
mod tests {
    ...
    /// A file lock has no lease to lapse, so it is never "lost" — the executor
    /// relies on this to keep writing fingerprints under purely local locking.
    #[tokio::test]
    async fn a_file_lock_is_never_lost() {
        let dir = tempdir();
        let lock = CacheLock::File {
            dir,
            timeout: Some(Duration::from_secs(2)),
        };
        let g = lock.lock("t", || {}).await.unwrap();
        assert!(!g.is_lost());
        g.unlock().await.unwrap();
    }

    /// A guard whose inner lease has been taken — what `unlock` leaves behind —
    /// reports no loss, so a released lock never suppresses a cache write. The
    /// held cases live with their backends (`ocicas`' `Lease::is_lost` tests and
    /// `redis_lock`'s `key_lost` table), which can drive a lease into the lost
    /// state; from here only the forwarding is observable.
    #[test]
    fn a_released_guard_is_not_lost() {
        assert!(!Guard::Vk(None).is_lost());
        assert!(!Guard::Redis(None).is_lost());
    }
